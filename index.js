...
function initGL() {
    ...
    /* Change camera perspective when mouse moves */
    ...
    enemyCounterDiv.innerText = `Enemies left: `
...
class Ship extends Entity{
    constructor(initialX, initialY, initialXv, initialYv, shapeData){
        ...
        this.hpBox = null
        // this.health = 100
        this.colors = initColors(this.points.length, 0.2, 0.6, 1, 1)
    ...
    immaFirinMyLasers(x, y){
        ...
        entities.forEach(e => {
            if(e instanceof Enemy){
                if(getDistanceFromPointToRay(e.x, e.y, this.x, this.y, x, y) <= e.sizeFactor)e.health -= 20
            }
        })
    }
}
...
class Enemy extends Entity{
    constructor(initialX, initialY, initialXv, initialYv, shapeData, sizeFactor) {
        ...
        this.health = 1000 * this.sizeFactor
...
class Laser extends Entity{
    constructor(initialX, initialY, initialXv, initialYv, shapeData, angle) {
        super(initialX, initialY, initialXv, initialYv, shapeData)
        this.colors = initColors(this.points.length, 1.0, 0.2, 0.2, 1)
